...
THIS SCRIPT PLOTS THE REGIONAL HG0 CONCENTRATIONS FOR EACH MONTH OF THE YEAR
...
def base(x,tit):
    ...
    m=Basemap(llcrnrlat=-47,  urcrnrlat=-2,
              llcrnrlon=10, urcrnrlon=160,
              resolution='c',projection='merc')
    #np.transpose because it mixes the lat lon order
    cs=m.pcolor(lons, lats, x, latlon=True, vmin=0.5, vmax=1.6, cmap=cm.plasma)#,
                #vmin=0,vmax=1300)
    m.drawcoastlines()
    ...
    plt.suptitle('Mean regional Hg$^0$ concentrations for each month', fontsize=15)
    return m
