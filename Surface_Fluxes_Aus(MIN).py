...
THIS SCRIPT PLOTS THE HG SURFACE FLUXES AT THE ASUTRALIA (MIN) SCALE
FOR EACH MONTH

THE MIN SCALE IS BOX COVERING GRID BOXES:
    26 to 40 LATITUDINAL
    118 to 133 LONGITUDINAL
...
area = dataset1.variables['DXYP__DXYP'][:,26:40,118:133]            # Grid box surface area (m2)
...
anthro1 = (dataset1.variables['HG_SRCE__Hg0_an'][:,26:40,118:133])  # Hg0 anthro emissions (kg/month)
anthro2 = (dataset1.variables['HG_SRCE__Hg2_an'][:,26:40,118:133])  # Hg2 anthro emissions (kg/month)
anthro3 = (dataset1.variables['HG_SRCE__HgP_an'][:,26:40,118:133])  # HgP anthro emissions (kg/month)
anthro = (anthro1 + anthro2 + anthro3)                # anthro emissions (kg/month)
biomass = (dataset1.variables['HG_SRCE__Hg_bb'][:,26:40,118:133])   # biomass emissions (kg/month)
soil = (dataset1.variables['HG_SRCE__Hg_so'][:,26:40,118:133])      # soil emissions (kg/month)
geogenic = (dataset1.variables['HG_SRCE__Hg0_na'][:,26:40,118:133]) # land natural emissions (kg/month)
land_leg = (dataset1.variables['HG_SRCE__Hg0_ln'][:,26:40,118:133]) # land legacy/re- emissions (kg/month)
ocean = (dataset1.variables['HG_SRCE__Hg0_oc'][:,26:40,118:133])    # ocean net emissions (kg/month)
hg0_dd=(dataset1.variables['DRYD_FLX__Hg0df'][:,26:40,118:133])     # Hg0 dry deposition flux (molec/cm2/s)
hgII_1=(dataset1.variables['WETDLS_S__Hg2'][:,:,26:40,118:133])       # Loss of Hg2 in precipitation (kg/s)
hgII_2=(dataset1.variables['WETDCV_S__Hg2'][:,:,26:40,118:133])       # Rainout loss of tracer in convective updrafts (kg/s)
hgII_wd = (hgII_1 + hgII_2)                           # HgII wet deposition flux (kg/s)
snow=(dataset1.variables['HG_SRCE__Hg0_snow'][:,26:40,118:133])     # Snow emission of Hg (kg)
...
plt.title("Australian Hg surface fluxes (Minimum Boundary)", fontsize=15)
