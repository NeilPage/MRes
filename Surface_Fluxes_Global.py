...
THIS SCRIPT PLOTS THE HG SURFACE FLUXES AT THE GLOBAL SCALE
FOR EACH MONTH
...
#------------------------------------------------------------------------------
# Define the dataset
dataset1 = MFDataset('/Users/ncp532/Documents/Some_scripts_/nc_Geogenic_Files/trac_avg.201*.nc') # Geogenic
...
area = dataset1.variables['DXYP__DXYP'][:]            # Grid box surface area (m2)
...
anthro1 = (dataset1.variables['HG_SRCE__Hg0_an'][:])  # Hg0 anthro emissions (kg/month)
anthro2 = (dataset1.variables['HG_SRCE__Hg2_an'][:])  # Hg2 anthro emissions (kg/month)
anthro3 = (dataset1.variables['HG_SRCE__HgP_an'][:])  # HgP anthro emissions (kg/month)
anthro = (anthro1 + anthro2 + anthro3)                # anthro emissions (kg/month)
biomass = (dataset1.variables['HG_SRCE__Hg_bb'][:])   # biomass emissions (kg/month)
soil = (dataset1.variables['HG_SRCE__Hg_so'][:])      # soil emissions (kg/month)
geogenic = (dataset1.variables['HG_SRCE__Hg0_na'][:]) # land natural emissions (kg/month)
land_leg = (dataset1.variables['HG_SRCE__Hg0_ln'][:]) # land legacy/re- emissions (kg/month)
ocean = (dataset1.variables['HG_SRCE__Hg0_oc'][:])    # ocean net emissions (kg/month)
hg0_dd=(dataset1.variables['DRYD_FLX__Hg0df'][:])     # Hg0 dry deposition flux (molec/cm2/s)
hgII_1=(dataset1.variables['WETDLS_S__Hg2'][:])       # Loss of Hg2 in precipitation (kg/s)
hgII_2=(dataset1.variables['WETDCV_S__Hg2'][:])       # Rainout loss of tracer in convective updrafts (kg/s)
hgII_wd = (hgII_1 + hgII_2)                           # HgII wet deposition flux (kg/s)
snow=(dataset1.variables['HG_SRCE__Hg0_snow'][:])     # Snow emission of Hg (kg)
...
plt.title("Global Hg surface fluxes", fontsize=15)
