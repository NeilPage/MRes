...
THIS SCRIPT PLOTS THE REGIONAL HG0 CONCENTRATION
...
wet2=(dataset1.variables['IJ_AVG_S__Hg2'][:])*(1e-3)*200.59/(6.0221*1e23)*airden1*1e6 # Hg0
wet3=(dataset1.variables['IJ_AVG_S__HgP'][:])*(1e-3)*200.59/(6.0221*1e23)*airden1*1e6 # Hg0
...
#avg1=wet1+wet2
avg1=wet1
wet1_avg=np.mean(avg1,axis=0)

avg2=wet2
wet2_avg=np.mean(avg2,axis=0)

#avg3=wet1+wet2
avg3=wet1+wet2
wet3_avg=np.mean(avg3,axis=0)
...
def plotkg(x, blah):
    ...
    #cs=m.pcolormesh(mlons, mlats, x, latlon=False, norm=colors.LogNorm(vmin=0.7, vmax=1.5), cmap=cm.plasma)   # define the colorbar attributes (colors used, add "_r" to flip the color scale)
    cs=m.pcolormesh(mlons, mlats, x, latlon=False, vmin=0.7, vmax = 1.3, cmap=cm.plasma)   # define the colorbar attributes (colors used, add "_r" to flip the color scale)    
    m.drawcoastlines()
...
#    m.drawmeridians(meridians,labels=[True,False,False,True], fontsize=10)
    ...
    x,y = m(lons, lats)
    m.scatter(x, y, c=obs, vmin=0.7, vmax = 1.3, cmap=cm.plasma, edgecolors='black', s=100)
    
    
    # Plot location labels
    labels = ['Cape Grim', 'Gunn Point', 'Glenville', 'Cape Point', 'Amsterdam Island']
    x_offsets = [-2500000, -2500000, -1000000, -30000, -3000000]
    y_offsets = [-250000, -150000, 250000, -550000, -650000]
    ...
    plt.title("Regional distribution of Hg$^0$ concentrations in the surface boundary layer", fontsize=15)
    return m
