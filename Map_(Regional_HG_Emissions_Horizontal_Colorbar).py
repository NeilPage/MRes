...
THIS SCRIPT PLOTS A MAP OF MEAN REGIONAL HG EMISSSIONS
WITH A HORIZONTAL COLORBAR
...
#wet4=(dataset1.variables['HG_SRCE__Hg_so'][:])   # soil emissions (kg)
...
def plotkg(x, blah):
    ...
    plt.title("TGM Anthropogenic Emissions", fontsize=15)
...
def plotkg1(x):
    ...
    cs=m.pcolor(lons, lats, x, latlon=True, vmin=0.1,
                norm=colors.LogNorm(), cmap=cm.plasma)   # define the colorbar attributes (colors used, add "_r" to flip the color scale)
            #, norm=colors.LogNorm())   # define the colorbar attributes (colors used, add "_r" to flip the color scale)
    ...
    for label, xpt, ypt, x_offset, y_offset in zip(labels, x, y, x_offsets, y_offsets):
        plt.text(xpt+x_offset, ypt+y_offset, label)
    # labels = [left,right,top,bottom]
    plt.xlabel('Longitude [$^\circ$]', fontsize=10, labelpad=20)
    plt.ylabel('Latitude [$^\circ$]', fontsize=10, labelpad=40)
    plt.title("TGM Biomass Burning Emissions", fontsize=15)
    return m

ax2=plt.subplot(gs[1]) # options graph 2 (vertical no, horizontal no, graph no)
#ax2 = plotkg(wet1_avg[0,:,:])
ax2 = plotkg(wet2_avg[:,:], 'Y')
